...
export const getOrganizationAppointmentsByDate = async ({
  ...
  try {
    ...
    const { dayStartUtc, dayEndUtc } = getLocalDayUtcRange(
      selectedDate,
      timeZone,
    );
    ...
    let query = supabase
      .from("appointments")
      .select(
        "id, organization_id, staff_id, client_name, client_phone, client_email, start_time, end_time, status, notes",
      )
...
export const createAppointment = async ({
  ...
  try {
    ...
    // --- ANTI BOTS / HOARDING LIMITS ---
    // 1 turno por día por negocio para el mismo cliente (basado en nombre y teléfono).
    ...
    // Limitamos por EMAIL de forma prioritaria (irrefutable si está logueado). Si no, por teléfono o por nombre.
    if (clientEmail?.trim()) {
      spamQuery = spamQuery.eq("client_email", clientEmail.trim());
    } else if (clientPhone?.trim()) {
      spamQuery = spamQuery.eq("client_phone", clientPhone.trim());
    } else {
      spamQuery = spamQuery.eq("client_name", clientName.trim());
    }

    const { data: existingAppts, error: queryErr } = await spamQuery;
    
    if (!queryErr && existingAppts && existingAppts.length > 0) {
      throw new Error("Ya tienes un turno reservado para este día en este local. Si deseas cambiar de horario, cancela el anterior primero.");
    }
    // ------------------------------------
    ...
    const { data, error } = await supabase
      ...
      .insert(payload)
      .select(
        "id, staff_id, start_time, end_time, client_name, client_phone, client_email, status",
      )
      .single();
...
export const updateAppointmentById = async ({ appointmentId, updates }) => {
  try {
    ...
    if (!updates || typeof updates !== "object")
      throw new Error("updates invalidos");
    ...
    const { data, error } = await supabase
      ...
      .is("deleted_at", null)
      .select(
        "id, staff_id, start_time, end_time, client_name, client_phone, status",
      )
      .single();
...
export const getClientAppointmentsByEmail = async (email) => {
  try {
      ...
      .select(`
        *,
        organizations:organization_id (
          id, name, logo_url, street, number,
    ...
    console.error("Error en getClientAppointmentsByEmail:", error);
    return { data: [], error };
  }
};
