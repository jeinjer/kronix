...
export default function SuperAdminTable({
  ...
  const statusStyles = {
    Activa: "bg-yellow-500/10 text-yellow-400 border-yellow-500/20",
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-slate-100 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/10">
            <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-500">
              Cliente & Proyecto
            </th>
            <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-500">
              Estado Suscripción
            ...
            <th className="p-6 text-xs font-black uppercase tracking-widest text-slate-500 text-right">
              Acciones de Gestión
            </th>
          </tr>
        </thead>
          ...
          <AnimatePresence>
            {suscripciones.map((s) => (
              ...
              >
                ...
                <td className="p-6">
                  <span
                    className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest border ${statusStyles[s.estado]}`}
                  >
                    {s.estado}
                  </span>
                </td>
                ...
                <td className="p-6">
                  <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300 font-mono text-sm">
                    ...
                    {s.fecha_limite_pago
                      ? new Date(s.fecha_limite_pago).toLocaleDateString()
                      : "N/A"}
                  </div>
                </td>
                ...
                <td className="p-6">
                  <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      ...
                      className="p-3 bg-yellow-500/10 text-yellow-400 rounded-xl hover:bg-yellow-500 hover:text-white transition-all"
