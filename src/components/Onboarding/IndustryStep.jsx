...
export default function IndustryStep({ formData, setFormData }) {
  return (
    ...
    >
      <div>
        <h2 className="text-2xl font-bold text-white mb-2">Segmentación</h2>
        <p className="text-slate-400">Configuración base del motor de KRONIX.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {industries.map((ind) => (
          <div 
            ...
            className={`relative p-5 rounded-xl border cursor-pointer transition-all duration-200 group ${formData.industry === ind.id ? `bg-slate-800 ${ind.border} ring-1 ring-offset-2 ring-offset-slate-900 ring-indigo-500` : 'bg-slate-950/40 border-slate-800 hover:bg-slate-900 hover:border-slate-700'}`}
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-4 bg-slate-900 border border-white/5`}>
              <ind.icon size={20} className={ind.color} />
            </div>
            <h3 className="text-white font-bold text-sm mb-1">{ind.label}</h3>
            <p className="text-xs text-slate-500 group-hover:text-slate-400 transition-colors">{ind.desc}</p>
