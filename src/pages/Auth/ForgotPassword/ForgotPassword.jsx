...
export default function ForgotPassword() {
  ...
  const cardClasses =
    "w-full max-w-md bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/10 rounded-[2rem] p-8 shadow-2xl transition-colors duration-300";
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 font-sans">
        <div className={`${cardClasses} text-center`}>
          ...
          <h2 className={`text-2xl font-black ${textMain} mb-2 tracking-tight`}>
            ¡Revisá tu bandeja!
          </h2>
          <p className={`${textMuted} text-sm mb-6 leading-relaxed`}>
            Te enviamos un enlace para restablecer tu contraseña. Si no lo ves,
            revisá la carpeta de Spam.
          </p>
          <Link
            to="/login"
            className="px-6 py-3 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-700 dark:text-slate-300 font-bold text-sm hover:bg-slate-200 dark:hover:bg-white/10 transition-colors"
          >
            Volver al inicio
          </Link>
        </div>
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 font-sans">
      <div className={`${cardClasses} relative overflow-hidden`}>
        ...
        <div className="relative z-10">
          <Link
            to="/login"
            className="inline-flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors mb-6"
          >
            <ArrowLeft size={14} /> Volver
          </Link>

          <h1
            className={`text-3xl font-black ${textMain} mb-2 uppercase tracking-tighter`}
          >
            Recuperar Acceso
          </h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-1">
              ...
              <div className="relative group">
                <Mail
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-cyan-600 dark:group-focus-within:text-cyan-400 transition-colors"
                  size={18}
                />
                <input
