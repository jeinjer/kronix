  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (orgErr) {
          console.error(
            "[SubscriptionGuard] Error leyendo organization_members:",
            orgErr,
          );
          if (mounted) setIsValid(true);
          return;
        ...
        const { data: statusObj, error: statusErr } = await supabase.rpc(
        ...
        if (statusErr) {
          console.error(
            "[SubscriptionGuard] Error en get_org_subscription_status:",
            statusErr,
          );
          if (mounted) setIsValid(true);
          return;
      ...
      <div className="h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center p-6 text-center transition-colors duration-500">
        <div className="max-w-md bg-white dark:bg-slate-950 p-8 rounded-3xl border border-slate-200 dark:border-white/10 transition-colors duration-500">
          ...
          <p className="text-slate-600 mb-6">
            Tu suscripción no está vigente. Contactate con administración para
