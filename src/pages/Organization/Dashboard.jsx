    ...
    <div className="max-w-7xl mx-auto px-4 py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          <button 
            ...
            className="p-2 hover:bg-slate-200 dark:hover:bg-white/5 rounded-xl text-slate-500 dark:text-slate-400 transition-colors"
          >
            <ChevronLeft size={24} />
          </button>
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">{currentOrg.name}</h1>
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Panel de Gestión</p>
          </div>
        </div>
        
        <button className="bg-slate-200 dark:bg-white/5 hover:bg-slate-300 dark:hover:bg-white/10 text-slate-900 dark:text-white px-4 py-2 rounded-xl text-sm font-bold border border-slate-300 dark:border-white/5 transition-all">
          Configuración
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/10 p-6 rounded-2xl transition-colors duration-500">
            <div className="flex justify-between items-start mb-4">
              <div className={`p-3 bg-slate-100 dark:bg-white/5 rounded-xl ${item.color}`}>
                <item.icon size={20} />
              </div>
      ...
      <div className="mt-8 bg-white dark:bg-[#13131a] border border-slate-200 dark:border-white/10 rounded-2xl p-8 min-h-[400px] flex flex-col items-center justify-center text-center transition-colors duration-500">
        <LayoutDashboard size={48} className="text-slate-500 dark:text-slate-800 mb-4" />
        <h2 className="text-xl font-bold text-slate-700 dark:text-slate-300">Bienvenido al control total</h2>
